import cv2, sys, os
import numpy as np
from glob import glob

### 기능
# 0. 파일 목록 읽기 (data 폴더) : *.jpg > list
# 1. 이미지 불러오기 
# 2. 마우스 콜백 함수 생성 
# 3. 콜백함수 안에서 박스를 그린다
# 4. 박스 좌표를 뽑아낸다 (마우스 좌표 2개)
#    (yolo) 박스 중심좌표(x,y),w,h
# 5. 이미지 파일명과 동일한 파일명으로 (확장자만 떼고) txt파일 생성


# (추가기능0) # 박스를 잘못 쳤을때, 'c'키를 누르면 현재 파일. 박스 내용 초기화
# (추가기능1) # 
# (추가기능2) # 화살표(->)를 누르면 다음 이미지로 로딩 (1~4)
# (추가기능3) # 화살표(<-)를 눌렀을 때, txt파일이 있다면 이미지 위에 띄워주기

############################################################################


# 0. 파일 목록 읽기 (data 폴더) : *.jpg > list

def getImageList():
    # 현재 작업 디렉토리 확인
    basePath = os.getcwd()
    dataPath = os.path.join(basePath,'images')
    fileNames = glob(os.path.join(dataPath,'*.jpg'))
    return fileNames


# 2개 좌표를 이용해서 직사각형 그리기
# corners : 좌표(startPt, EndPt)
def drawROI(img, corners):
    cpy = img.copy()    # 박스 그릴 레이어 (cpy)
    line_c = (128,128,255) # 직선의 색상
    lineWidth = 2   # 선 굵기
    cv2.rectangle(cpy, tuple(corners[0]), tuple(corners[1]), color=line_c, thickness=lineWidth)
    # Lbuttondown > mousemove

    # alpha=0.3 + beta=0.7 + gamma=0 = sum=1
    disp = cv2.addWeighted(img, 0.3, cpy, 0.7, 0)
    return disp



## 2. 마우스 콜백 함수 정의
# 박스를 그릴 때 start - end 좌표값을 담아줄 함수 
def onMouse(event, x, y, flags, param):
    # 좌표값은 저장-읽기-변경 되어야하므로 전역변수로 설정
    global startPt, img, ptList, cpy, txtWrData

    if event == cv2.EVENT_LBUTTONDOWN:  # 마우스 버튼 누를 때 x,y좌표 담기
        startPt = (x,y)
    elif event == cv2.EVENT_LBUTTONUP:  # 마우스 버튼 땔 때 x,y좌표 담기
        ptList = [startPt, (x,y)]
        txtWrData = str(ptList)         # txt data 저장
        cpy = drawROI(img, ptList)
        startPt = None
        cv2.imshow('label', cpy)
    elif event == cv2.EVENT_MOUSEMOVE:
        if startPt:
            ptList = [startPt, (x,y)]
            cpy = drawROI(img, ptList) # 전역변수 cpy 수정
            cv2.imshow('label', cpy)

    
# 마우스가 눌리지 않으면 좌표값: None
ptList = []
startPt=None
cpy=[]
txtWrData = None


# 1. 이미지 불러오기 

fileNames = getImageList()
img = cv2.imread(fileNames[0])
cv2.namedWindow('label')


# 콜백 함수 호출
cv2.setMouseCallback('label',onMouse,[img])
    # windowName, onMouse, parameter
cv2.imshow('label', img)
 
# 종료 조건 추가 (ESC)
while True :
    key = cv2.waitKey()
    if key == 27:
        break

    elif key == ord('s'):
        fileNames, ext = os.path.splitext(fileNames[0]) 
        txtFileName = fileNames + '.txt'
        f = open(txtFileName, 'w')
        f.write(txtWrData)
        f.close()
        # print("before write txt: {}".fotmat(txtWrData))

cv2. destroyAllWindows()